...
data = readtable('post_filtering.csv', 'Delimiter', ';'); % Load the CSV file with semicolon delimiter
...
total_time = 13; % Total time in seconds
...
if all(ismember({'x_f', 'y_f', 'z_f', 'x', 'y', 'z'}, data.Properties.VariableNames))
    ...
    writetable(data, 'post_filtering_with_time.csv'); % Save to new CSV file
